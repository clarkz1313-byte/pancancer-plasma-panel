#!/usr/bin/env python3
"""
fig2b_npx_violin.py — Per-sample median NPX distribution per cancer
For each sample, compute the median NPX across all 1,463 proteins → one scalar.
Then violin + embedded box + jittered strip of those per-sample medians per cancer.
This shows whether cancers differ in GLOBAL plasma protein abundance,
avoiding the "all-proteins-pooled identical violins" artifact.
Output: figurev5/output/fig2b_npx_violin.pdf/.png
"""
...
# Per-sample median NPX across all 1,463 proteins
df["median_npx"] = df[prot_cols].median(axis=1)

data_per_cancer = []
for c in CANCERS:
    vals = df[df["Cancer"] == c]["median_npx"].dropna().values
    data_per_cancer.append(vals)
    print(f"  {c}: n={len(vals)}  median={np.median(vals):.3f}  "
...
ax.set_ylabel("Per-sample median NPX  (log2 arbitrary unit)",
              fontsize=13, fontweight="bold", labelpad=6)
ax.set_title(
    "Global plasma protein abundance per cancer type\n"
    "Median NPX across 1,463 proteins per sample · 1,375 samples",
