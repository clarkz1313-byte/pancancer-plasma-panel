...
enable_text_scaling(factor=1.35, minimum=12.0, axis_label_scale=0.75)
...
TRAIT_LABEL = {"pQTL": "BMP4 pQTL", "GWAS": "CRC GWAS"}
LABELS_BMP4 = {"pQTL": "BMP4 pQTL", "GWAS": "CRC GWAS"}
LABELS_LEP  = {"pQTL": "LEP pQTL",  "GWAS": "BRC GWAS"}
...
C_FIND = "#6a1b9a"       # chr11 / BMP4 x CRC
C_REGION16 = "#008c95"    # chr16 / LEP x BRC
C_CTRL = "#d68600"       # chr14 / BMP4 cis control
...
def _panel_heading(ax, letter, text, color=C_INK):
    """Use one concise panel-letter system; interpretation stays in caption."""
    ax.text(0.0, 1.035, letter, transform=ax.transAxes, ha="left", va="bottom",
            fontsize=20, fontweight="bold", color=C_INK, clip_on=False)
...
def panel_genome(ax, man):
    ...
    alt = man["chr"] % 2 == 0
    ax.scatter(man.loc[alt, "xcum"], man.loc[alt, "logp"], s=5.0, c="#c3ccd4",
               linewidths=0, rasterized=True, zorder=2)
    ax.scatter(man.loc[~alt, "xcum"], man.loc[~alt, "logp"], s=5.0, c="#8796a3",
               linewidths=0, rasterized=True, zorder=2)
    ...
    for chrom, mb, col, name, ptr, ytop in [
            (11, 74.716876, C_FIND, "chr11 CRC", "C", 0.97),
            (14, 53.953000, C_CTRL, "chr14 CRC", "E", 0.62)]:
        ...
        ax.annotate(f"{ptr} · {name}",
                    xy=(centre, min(best, ymax * 0.94)), xytext=(centre, ymax * ytop),
                    ha="center", va="top", fontsize=13, fontweight="bold",
                    color=col, zorder=6, linespacing=1.35,
                    arrowprops=dict(arrowstyle="-|>", color=col, lw=1.5,
                                    shrinkA=3, shrinkB=4))

    # No whole-genome BRC Manhattan is available in the repository.  The
    # chr16 band therefore locates panel D on the shared genome coordinate
    # frame; it does not encode a CRC association.
    centre16 = LEP_INSTRUMENT + offs[16]
    ax.axvspan(centre16 - 7e6, centre16 + 7e6,
               color=C_REGION16, alpha=0.22, zorder=1)
    ax.annotate("D · chr16 BRC",
                xy=(centre16, ymax * 0.52), xytext=(centre16, ymax * 0.82),
                ha="center", va="top", fontsize=13, fontweight="bold",
                color=C_REGION16, zorder=6,
                arrowprops=dict(arrowstyle="-|>", color=C_REGION16, lw=1.5,
                                shrinkA=3, shrinkB=4))
    ...
    ax.set_xticks([offs[c] + (man[man["chr"] == c]["bp"].max() / 2)
                   for c in range(1, 23)])
    ax.set_xticklabels([str(c) for c in range(1, 23)], fontsize=12)
    ax.set_xlabel("Chromosome", fontsize=15, labelpad=5)
    ax.set_ylabel("CRC GWAS\n$-\\log_{10}\\ p$", fontsize=15)
    ax.tick_params(axis="y", labelsize=13)
    ax.spines[["top", "right"]].set_visible(False)
    # Boundary triangles disclose above-cap SNPs. The exact count is reported
    # in the caption, keeping the Manhattan panel visual and uncluttered.
...
def _track(ax, s, src, probe_pos, probe_col, show_x, chrom, ylim_top,
    ...
    s["bin"] = pd.cut(dkb, bins=[-np.inf, 50, 100, 200, 350, np.inf],
                      labels=BINS)
    cap_y = ylim_top * 0.94
    s["plot_y"] = s["neglog10p"].clip(upper=cap_y)
    for key in BINS[::-1]:
        m = s[s["bin"] == key]
        if len(m):
            ax.scatter(m["pos"] / 1e6, m["plot_y"], s=22,
                       c=DIST_COL[key], linewidths=0, alpha=0.9,
                       zorder=3 + BINS.index(key))
            clipped = m[m["neglog10p"] > cap_y]
            if len(clipped):
                ax.scatter(clipped["pos"] / 1e6, np.full(len(clipped), cap_y),
                           s=34, marker="^", c=DIST_COL[key], linewidths=0,
                           zorder=9)
    ...
    if len(s):
        ...
        ax.scatter([probe_pos / 1e6], [min(at, cap_y)], s=150, marker="D",
    ...
    ax.text(0.985, 0.955, (labels or TRAIT_LABEL)[src], transform=ax.transAxes,
            ha="right", va="top", fontsize=15, fontweight="bold",
            color=tcol, zorder=11)
    ax.text(0.985, 0.81, f"$p$={10 ** -at:.0e}",
            transform=ax.transAxes, ha="right", va="top", fontsize=14,
            fontweight="bold", color=tcol, zorder=11)

    ax.set_ylim(0, ylim_top)
    ax.set_ylabel(f"{src}\n$-\\log_{{10}}\\ p$", fontsize=14, color=tcol)
    ax.tick_params(axis="y", colors=tcol, labelsize=13)
    ax.spines["left"].set_color(tcol)
    ax.spines["left"].set_linewidth(2.0)
    ax.spines[["top", "right"]].set_visible(False)
    ax.grid(color="#ffffff", lw=0.8, zorder=0)
    ax.set_axisbelow(True)
    ax.tick_params(axis="x", labelsize=12, colors=probe_col)
    if show_x:
        ax.set_xlabel(f"chr{chrom} position (Mb)", fontsize=14,
                      color=probe_col)
    else:
...
def _genes(ax, spans):
    ...
    for index, (name, (g0, g1)) in enumerate(spans.items()):
        col = GENE_COL[name]
        ax.axvspan(g0, g1, color=col, alpha=0.15, zorder=0)
        ax.text((g0 + g1) / 2, ax.get_ylim()[1] * (0.78 - 0.08 * index), name, ha="center",
...
def _verdict(ax, text, sub, color):
    ax.text(0.022, 0.955, text, transform=ax.transAxes, ha="left", va="top",
            fontsize=12, fontweight="bold", color=color, zorder=20)
    if sub:
        ax.text(0.028, 0.79, sub, transform=ax.transAxes, ha="left", va="top",
            fontsize=12, fontweight="bold", color=color, zorder=20)
...
def panel_decision_plane(ax, scr):
    ...
    ax.axhspan(-np.log10(bonf), 25, xmin=(0.80 + 0.02) / 1.04, color=C_FIND,
               alpha=0.08, zorder=0)
    ax.scatter(d.loc[rest, "PP.H4"], d.loc[rest, "y"], s=25, c="#c3ccd4",
               edgecolors="none", alpha=0.85, zorder=2,
               label="Other")
    ax.scatter(d.loc[near, "PP.H4"], d.loc[near, "y"], s=110, c="#c47a2c",
               edgecolors="white", linewidths=0.8, zorder=3,
               label="Wald")
    ax.scatter(d.loc[promoted, "PP.H4"], d.loc[promoted, "y"], s=330,
    ...
    ax.text(0.03, -np.log10(bonf) + 0.9, f"$p$ < {bonf:.1e}",
    ...
    for lab, dx, dy_ in [("BMP4 × CRC", -100, 6), ("LEP × BRC", -120, -30)]:
        prot = lab.split(" ")[0]
    ...
    ax.set_ylim(-0.5, 25)
    ax.set_xlabel("Colocalization posterior (PP.H4)", fontsize=15,
                  fontweight="bold")
    ax.set_ylabel("Wald  $-\\log_{10}\\ p$", fontsize=15,
                  fontweight="bold")
    ax.tick_params(axis="both", labelsize=13)
    ax.spines[["top", "right"]].set_visible(False)
    ax.legend(loc="upper left", frameon=False, fontsize=13, ncol=3,
...
def main():
    ...
    # A compact evidence screen and a genome-wide CRC overview establish the
    # selection and genomic context.  The lower row then zooms into the two
    # promoted regions and the BMP4 cis control.  Only the CRC overview is
    # genome-wide because no whole-genome BRC source exists in this package.
    gs = gridspec.GridSpec(2, 6, figure=fig,
                           height_ratios=[0.62, 1.38],
                           hspace=0.31, wspace=0.54,
                           left=0.052, right=0.985, top=0.955, bottom=0.055)

    ax_a = fig.add_subplot(gs[0, 0:2])
    panel_decision_plane(ax_a, scr)
    _panel_heading(ax_a, "A", "Screen")

    ax_b = fig.add_subplot(gs[0, 2:6])
    panel_genome(ax_b, man)
    _panel_heading(ax_b, "B", "Genome + locus locators")
    ...
    lq, lg = load_lep()
    sub_c = gs[1, 0:2].subgridspec(2, 1, hspace=0.10)
    ax_c1, ax_c2 = fig.add_subplot(sub_c[0]), fig.add_subplot(sub_c[1])
    _track(ax_c1, tq, "pQTL", CHR11_INSTRUMENT, C_FIND, False, 11, y_q)
    _track(ax_c2, tg, "GWAS", CHR11_INSTRUMENT, C_FIND, True, 11, y_g)
    ax_c2.sharex(ax_c1)
    for a in (ax_c1, ax_c2):
        _genes(a, {"CHRDL2": CHRDL2_SPAN_MB, "POLD3": POLD3_SPAN_MB})
    _verdict(ax_c1, "HEIDI p  .004 / .075", "PP.H4  .99→.66",
             C_FIND)
    _panel_heading(ax_c1, "C", "BMP4–CRC · chr11", C_FIND)

    sub_e = gs[1, 2:4].subgridspec(2, 1, hspace=0.10)
    ax_e1, ax_e2 = fig.add_subplot(sub_e[0]), fig.add_subplot(sub_e[1])
    _track(ax_e1, lq, "pQTL", LEP_INSTRUMENT, C_REGION16, False, 16, y_q,
           labels=LABELS_LEP)
    l_at = _track(ax_e2, lg, "GWAS", LEP_INSTRUMENT, C_REGION16, True, 16, y_g,
                  labels=LABELS_LEP)
    ax_e2.sharex(ax_e1)
    for a in (ax_e1, ax_e2):
        _genes(a, {"FTO": FTO_SPAN_MB})
    _verdict(ax_e1, "HEIDI p  .011 / .055", "PP.H4  .99→.43",
             C_REGION16)
    _panel_heading(ax_e1, "D", "LEP–BRC · chr16", C_REGION16)
    ...
    _verdict(ax_f1, "No overlap", "", C_CTRL)
    _panel_heading(ax_f1, "E", "BMP4 cis · chr14", C_CTRL)


    for ext in ("png", "pdf"):
        p = OUT / f"fig13_regional_evidence_2026-09-05.{ext}"
